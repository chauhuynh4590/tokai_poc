...
# nt_bang: say YOLO again
class YoloModel:
    def __init__(self):
        self.device = Config.DEVICE
        self.model = None
        self.version = "v0.2"
        self.model_path = Config.VERSION_LIST
        self.ov_core = Core()

    def get_version_list(self):
        return list(self.model_path.keys())

    def get_first_version(self):
        return list(self.model_path.keys())[0]

    def load(self, version="v0.2"):
        self.model = self.ov_core.read_model(self.model_path[version])
        if self.device != "CPU":
            self.model.reshape({0: [1, 3, 640, 640]})

        self.model = self.ov_core.compile_model(self.model, self.device)
...
def draw_results(results: Dict, source_image: np.ndarray, label_map: Dict):
    ...
    for idx, box in enumerate(boxes):  # (*xyxy, conf, lbl)
        print("ox", box)
        xyxy = (box[0:4])
        conf = box[5]
        lbl = box[4]
        ...
        source_image = plot_one_box(xyxy, source_image, label=label, color=colors(int(lbl)),
                                    line_thickness=1)
    return source_image
