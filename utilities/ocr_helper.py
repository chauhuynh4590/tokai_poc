...
import utilities.pre_post_processing as processing
...
class PDOCR_MODEL:
    def __init__(self):
        core = Core()
        det_model = core.read_model(model=Config.PDOCR_DET_MODEL)
        self.det_compiled_model = core.compile_model(model=det_model, device_name=Config.DEVICE)

        # Get input and output nodes for text detection.
        self.det_input_layer = self.det_compiled_model.input(0)
        self.det_output_layer = self.det_compiled_model.output(0)

        rec_model = core.read_model(model=Config.PDOCR_REC_MODEL)
        ...
        self.rec_compiled_model = core.compile_model(model=rec_model, device_name=Config.DEVICE)

        # Get input and output nodes.
        self.rec_input_layer = self.rec_compiled_model.input(0)
        self.rec_output_layer = self.rec_compiled_model.output(0)

    def run_paddle_ocr(self, frame=None):
        """
        ...
        Parameters:
            frame: The webcam number to feed the video stream with primary webcam set to "0", or the video path.
        """
